...
function updateSuperAdmin(dir) {
    if (!fs.existsSync(dir)) return;
    const files = fs.readdirSync(dir);
    for (const file of files) {
        const fullPath = path.join(dir, file);
        if (fs.statSync(fullPath).isDirectory()) {
            updateSuperAdmin(fullPath);
            ...
            if (oldScriptRegex.test(html)) {
                ...
                console.log('Updated Super Admin file:', fullPath);
            }
        }
    }
}

updateSuperAdmin(path.join(__dirname, '.next', 'server', 'app', 'super-admin'));
updateSuperAdmin(path.join(__dirname, '.next', 'server', 'app', 'super-admin.html'));
