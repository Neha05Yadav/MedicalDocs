    ...
    <Shell eyebrow="Diagnostics marketplace" title="Book laboratory tests" description="Compare your laboratory’s published tests and packages, see preparation instructions and pricing, and request home sample collection.">
      <div className="grid gap-6 xl:grid-cols-[1.35fr_.65fr]">
        <section className="rounded-[1.5rem] border border-slate-200/80 bg-white p-5 shadow-[0_16px_45px_-32px_rgba(15,23,42,.35)] md:p-6">
          ...
          {!!catalog.tests.length && <div className="mt-8"><div className="mb-4 flex items-end justify-between gap-3"><div><div className="flex items-center gap-2"><TestTube2 className="size-5 text-cyan-600" /><h2 className="font-black text-slate-900">Available lab tests</h2></div><p className="mt-1 text-xs text-slate-500">Select individual tests to add to your order.</p></div><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600">{catalog.tests.length} tests</span></div><div className="grid gap-4 md:grid-cols-2">{catalog.tests.map((test: any) => { const isAdded = testIds.includes(test.id); return <article key={test.id} className={`group flex min-h-[230px] flex-col rounded-[1.25rem] border bg-white p-5 transition-all duration-200 hover:-translate-y-1 hover:shadow-[0_18px_38px_-24px_rgba(8,145,178,.45)] ${isAdded ? "border-cyan-400 ring-2 ring-cyan-100" : "border-slate-200 hover:border-cyan-200"} ${packageId ? "opacity-60" : ""}`}><div className="flex items-start justify-between gap-4"><div className="flex min-w-0 items-center gap-3"><span className={`grid size-10 shrink-0 place-items-center rounded-xl ${isAdded ? "bg-cyan-600 text-white" : "bg-cyan-50 text-cyan-700"}`}><TestTube2 className="size-5" /></span><div className="min-w-0"><p className="truncate text-[15px] font-black text-slate-900">{test.name}</p><p className="mt-0.5 text-xs font-medium text-slate-500">{test.sampleType || "Diagnostic test"}</p></div></div><p className="shrink-0 text-lg font-black text-cyan-700">{money(test.price)}</p></div><div className="mt-5 grid gap-3 text-xs"><div className="flex items-center gap-2 rounded-xl bg-slate-50 px-3 py-2.5 text-slate-600"><Clock3 className="size-4 shrink-0 text-cyan-600" /><span><strong className="text-slate-800">Report time:</strong> {test.turnaroundHours ? `${test.turnaroundHours} hours` : "Contact laboratory"}</span></div><div className="flex items-start gap-2 rounded-xl bg-amber-50/70 px-3 py-2.5 leading-5 text-amber-800"><ShieldCheck className="mt-0.5 size-4 shrink-0" /><span><strong>Preparation:</strong> {test.preparationInstructions || "No special preparation required"}</span></div></div><button type="button" disabled={Boolean(packageId)} onClick={() => setTestIds(isAdded ? testIds.filter((id) => id !== test.id) : [...testIds, test.id])} className={`mt-auto flex w-full items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-black transition-all disabled:cursor-not-allowed ${isAdded ? "bg-cyan-50 text-cyan-700 hover:bg-cyan-100" : "bg-slate-950 text-white hover:bg-cyan-700"}`}><Plus className={`size-4 transition-transform ${isAdded ? "rotate-45" : "group-hover:rotate-90"}`} />{isAdded ? "Remove" : "Add"}</button></article>; })}</div></div>}
        </section>
        <aside className="h-fit rounded-[1.5rem] border border-slate-800 bg-slate-950 p-6 text-white shadow-[0_24px_55px_-28px_rgba(15,23,42,.9)] xl:sticky xl:top-6"><div className="mb-5 flex items-center gap-3"><span className="grid size-11 place-items-center rounded-2xl bg-cyan-400/10 text-cyan-300 ring-1 ring-cyan-300/15"><FlaskConical className="size-6" /></span><div><h2 className="text-lg font-black">Order summary</h2><p className="mt-0.5 text-sm text-slate-400">{selectedPackage ? selectedPackage.name : `${selectedTests.length} individual test(s)`}</p></div></div><div className="my-5 space-y-4 border-y border-slate-800 py-5"><Field label="Priority"><select className="care-input text-slate-900" value={priority} onChange={(e) => setPriority(e.target.value)}><option>Normal</option><option>High</option><option>Urgent</option></select></Field><label className="flex cursor-pointer items-center justify-between gap-3 rounded-xl border border-slate-800 bg-white/[.03] p-3 text-sm transition hover:border-slate-700"><span>Home sample collection</span><input type="checkbox" checked={homeCollection} onChange={(e) => setHomeCollection(e.target.checked)} className="size-4 accent-cyan-400" /></label>{homeCollection && <Field label="Collection address"><textarea className="care-input text-slate-900" value={collectionAddress} onChange={(e) => setCollectionAddress(e.target.value)} /></Field>}</div><div className="mb-5 flex items-end justify-between gap-4 rounded-2xl bg-white/[.04] p-4"><span className="text-sm text-slate-300">Estimated total</span><span className="text-2xl font-black tracking-tight text-cyan-300">{money(total)}</span></div><button onClick={book} disabled={!laboratoryId || (!packageId && !testIds.length)} className="w-full rounded-xl bg-cyan-400 py-3.5 font-black text-slate-950 shadow-[0_12px_28px_-14px_rgba(34,211,238,.8)] transition hover:-translate-y-0.5 hover:bg-cyan-300 disabled:translate-y-0 disabled:opacity-40">Place lab order</button></aside>
      </div>
    </Shell>
  );
}
