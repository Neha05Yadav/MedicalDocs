...
const BookOpen = (props: any) => <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M12 7v14"></path><path d="M3 18a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h5a4 4 0 0 1 4 4 4 4 0 0 1 4-4h5a1 1 0 0 1 1 1v13a1 1 0 0 1-1 1h-6a3 3 0 0 0-3 3 3 3 0 0 0-3-3z"></path></svg>;
...
const IconMap: Record<string, React.ElementType> = {
  LayoutDashboard, ShieldCheck, Users, Hospital, CreditCard, 
  ClipboardList, Bell, FileText, Settings, Stethoscope, 
  FlaskConical, Calendar, Pill, User, Clock, BookOpen, 
  RefreshCw, Headset, MessageSquare, AlertTriangle, Activity, 
  BarChart3, Search
};
// Server component approach:
export default function Sidebar({ collapsed, currentPath, activeNav }: { collapsed: boolean, currentPath: string, activeNav: any[] }) {
  ...
  return (
    <aside
      className={`hidden md:flex shrink-0 border-r border-border bg-gradient-to-br from-blue-50 to-white flex-col transition-all duration-300 ${
        collapsed ? "w-16" : "w-64"
      }`}
    >
        ...
        {!collapsed && (
          <div className="flex flex-col">
             ...
             <span className="text-[10px] text-muted-foreground font-medium uppercase tracking-wider mt-0.5">
               {isSuperAdmin ? "Super Admin" : isAccounts ? "Accounts" : isSales ? "Sales" : isSupport ? "Support" : isAdmin ? "Admin" : isHospital ? "Hospital" : isLaboratory ? "Laboratory" : isClinic ? "Clinic" : "Patient"} Dashboard
             </span>
          </div>
        )}
      </div>
      <nav className="flex-1 p-3 space-y-1 overflow-y-auto no-scrollbar">
        {activeNav.map((item) => {
          ...
          return (
            <SidebarLink key={item.title} url={item.url} title={item.title} collapsed={collapsed}>
