...
export default function DoctorReportsPage() {
  const [activeTab, setActiveTab] = useState("clinic"); // "clinic" | "lab" | "upload"
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-2 bg-slate-100 p-1.5 rounded-xl">
          <button 
            ...
            className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${activeTab === "clinic" ? "bg-white text-slate-800 shadow-sm" : "text-slate-500 hover:text-slate-700 hover:bg-slate-200"}`}
          >
            Clinic Reports
          </button>
          <button 
            onClick={() => setActiveTab("lab")}
            className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${activeTab === "lab" ? "bg-white text-slate-800 shadow-sm" : "text-slate-500 hover:text-slate-700 hover:bg-slate-200"}`}
          >
