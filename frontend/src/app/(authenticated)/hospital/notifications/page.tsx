...
import { 
  Bell, FileText, CheckCircle2, ShieldAlert, Activity, User, Trash2, Hospital
} from "lucide-react";
...
export default function HospitalNotificationsPage() {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case "System": return <Activity className="size-5 text-blue-600" />;
  ...
  const getIconBg = (type: string) => {
    switch (type) {
      ...
      case "Request": return "bg-emerald-50";
    ...
    <div className="p-8 max-w-4xl mx-auto w-full min-h-screen">
      <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Hospital alerts, staff updates, and system notifications.</p>
        </div>
