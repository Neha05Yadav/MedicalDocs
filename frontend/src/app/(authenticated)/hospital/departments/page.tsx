...
const UserCheck = (props: any) => <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="m16 11 2 2 4-4"></path><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle></svg>;
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto w-full min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mt-10 mb-6 gap-4">
        ...
        <div className="flex items-center gap-2 text-sm text-slate-500 relative">
          <span>Filter by:</span>
          <div 
            onClick={() => setIsDropdownOpen(!isDropdownOpen)}
            className="flex items-center justify-between min-w-[160px] bg-white border border-slate-200 rounded-lg px-3 py-2 cursor-pointer shadow-sm hover:bg-slate-50 transition-colors"
          >
            <span className="font-medium text-slate-700 truncate mr-2">{selectedDepartment}</span>
            <ChevronDown className={`size-4 transition-transform ${isDropdownOpen ? 'rotate-180' : ''}`} />
          </div>
          {isDropdownOpen && (
            <div className="absolute top-full right-0 mt-2 w-48 bg-white border border-slate-200 rounded-xl shadow-lg z-10 overflow-hidden py-1">
              <button 
                onClick={() => { setSelectedDepartment("All Departments"); setIsDropdownOpen(false); }}
                className={`w-full text-left px-4 py-2 text-sm transition-colors ${selectedDepartment === "All Departments" ? "bg-cyan-50 text-[#0891b2] font-semibold" : "text-slate-700 hover:bg-slate-50"}`}
              >
                All Departments
              </button>
              {departmentsData.map(dept => (
                <button 
                  key={dept.name}
                  onClick={() => { setSelectedDepartment(dept.name); setIsDropdownOpen(false); }}
                  className={`w-full text-left px-4 py-2 text-sm transition-colors ${selectedDepartment === dept.name ? "bg-cyan-50 text-[#0891b2] font-semibold" : "text-slate-700 hover:bg-slate-50"}`}
                >
                  {dept.name}
                </button>
              ))}
        ...
        {loading ? (
          ...
          <div key={dept.name} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 hover:shadow-md transition-shadow flex flex-col items-center">
            ...
            <div className="w-full flex justify-between px-2 mb-4">
              <div className="flex flex-col items-center">
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-sm font-bold text-slate-900">{dept.doctors}</span>
                </div>
            ...
            <div className="flex flex-col items-center mb-6">
              <div className="flex items-center gap-1.5 mb-1">
                <FileText className="size-4 text-purple-500" strokeWidth={2.5} />
                <span className="text-sm font-bold text-slate-900">{dept.reports}</span>
              </div>
              <span className="text-[9px] text-slate-400 uppercase tracking-wider font-bold">Reports</span>
            </div>
            <div className="w-full border-t border-dashed border-slate-200 pt-5 flex justify-center">
              <span className="px-6 py-1 bg-white text-emerald-600 font-bold text-xs rounded-full">
                Active
              </span>
            </div>
          </div>
