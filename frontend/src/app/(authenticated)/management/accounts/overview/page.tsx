...
export default function AccountsOverviewPage() {
  ...
  useEffect(() => {
    fetch('/api/management/accounts/overview')
      .then(res => res.json())
      .then(data => {
        if (data.kpi) {
          setKpiData([
            { title: "Total Income (All time)", value: data.kpi.totalIncome, change: "+24.5%", isPositive: true, icon: Landmark, bgColor: "bg-indigo-50", iconColor: "text-indigo-600" },
            { title: "Total Collected Amount", value: data.kpi.totalCollected, change: "+18.2%", isPositive: true, icon: Wallet, bgColor: "bg-emerald-50", iconColor: "text-emerald-600" },
            { title: "Pending (Receivable)", value: data.kpi.pendingReceivable, change: "-5.4%", isPositive: true, icon: Activity, bgColor: "bg-amber-50", iconColor: "text-amber-600" },
            { title: "Overdue Amount", value: data.kpi.overdueAmount, change: "+12.1%", isPositive: false, icon: AlertCircle, bgColor: "bg-rose-50", iconColor: "text-rose-600" },
            { title: "Refund Issued", value: data.kpi.refundIssued, change: "-2.5%", isPositive: true, icon: RefreshCcw, bgColor: "bg-orange-50", iconColor: "text-orange-600" },
          ]);
        }
        if (data.revenueData) {
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex flex-col">
          ...
          <div className="space-y-4 flex-1">
            <Link href="/management/accounts/invoicing-system" className="w-full flex items-center justify-between p-4 rounded-xl border border-slate-200 hover:border-indigo-300 hover:bg-indigo-50/50 transition-colors group">
            ...
            <Link href="/management/accounts/billing-records" className="w-full flex items-center justify-between p-4 rounded-xl border border-slate-200 hover:border-emerald-300 hover:bg-emerald-50/50 transition-colors group">
              <div className="flex items-center gap-3">
            ...
            <Link href="/management/accounts/pending-payments" className="w-full flex items-center justify-between p-4 rounded-xl border border-slate-200 hover:border-amber-300 hover:bg-amber-50/50 transition-colors group">
              <div className="flex items-center gap-3">
