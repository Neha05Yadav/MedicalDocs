      ...
      <div className="mx-auto max-w-[1600px]">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {metrics(area, data).map((item: any) => (
            <article key={item.label} className="group rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition duration-200 hover:border-slate-300 hover:shadow-md">
              <div className="flex items-start justify-between gap-4"><div><p className="text-[11px] font-black uppercase tracking-[.12em] text-slate-500">{item.label}</p><p className="mt-3 text-[2rem] font-black tracking-tight text-slate-950">{item.value}</p></div><span className="rounded-xl border border-slate-200 bg-slate-50 p-2.5 text-slate-600"><item.icon className="size-5" /></span></div>
              <p className="mt-4 border-t border-slate-100 pt-3 text-xs font-medium text-slate-500">{item.note}</p>
            </article>
          ))}
        </div>
