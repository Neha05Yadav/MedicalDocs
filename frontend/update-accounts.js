...
function updateAccounts(dir) {
    if (!fs.existsSync(dir)) return;
    const files = fs.readdirSync(dir);
    for (const file of files) {
        const fullPath = path.join(dir, file);
        if (fs.statSync(fullPath).isDirectory()) {
            updateAccounts(fullPath);
            ...
            if (oldScriptRegex.test(html)) {
                ...
                console.log('Updated Accounts file:', fullPath);
            }
        }
    }
}

updateAccounts(path.join(__dirname, '.next', 'server', 'app', 'accounts'));
updateAccounts(path.join(__dirname, '.next', 'server', 'app', 'accounts.html'));
